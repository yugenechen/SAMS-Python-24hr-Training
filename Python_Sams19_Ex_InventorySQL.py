...
#         e.4) quantity
#         e.5) Food Category
#       [Status]: Done, editItem(conn, itemName)
...
#      h)  Consolidate Duplicates - tally up qty of duplicates, update the
           ...
           duplicates.
        [Status]: Done, see consolidateInvItem(conn, itemName).
...
def open_db(dbName, *path):
    ...
    if not path:
        ...
        # converts the fullPath from a Tuple to a string
        # Convert single char tuple making up *path (that is an *arg) into a
        # full text string.
        fullPath = ""
        for char in path:
            fullPath += char
        ...
        if fullPath[length] in ('\\', '/'):
            fullPathFileName = fullPath + '\\' + dbName
        fullPathFileName = fullPath + dbName
        ...
        while addItemsManually:
            while not itemName:
                ...
                if not itemName:
                    ...
                    # Done entering data so exit out of the function. Could
                    # have just used "return True" but I prefer the longer drop
                    # through method when quitting the loop.  It allows for
                    # more common handling and I like having one return if
                    # it is easy to implement.  I think it makes for more
                    # consistent code for better future troubleshooting.
                    # Still a normal execution (quit) so return True.
                    status = True
                    # Set False to fall out of the parent loop
                    addItemsManually = False
            ...
            if (itemName != 'quit') and not duplicate:
                # create an item list for the write segment
                item = [(itemName, unit, qty, FCat)]
            elif duplicate:
...
def checkForItem(conn, searchItem=""):
    """Decrement the quantity of an inventory 'item' in the SQLite database.
    ...
    OUTPUT:
      :print to screen:    - prints to screen if 'toScreen' is True
      :return (tuple/str): - If searchItem in inventory then return a tuple. \
...
def editItem(conn, itemName):
    sqlEditQuery = ('''Select itemName, unit, qty, FoodCategory, Desc
                    FROM Inventory
                    WHERE itemName = :itemName''',
                    {'itemName': itemName})
    resultsEditQuery = getSqlData(conn, sqlEditQuery)
    resultDict = {resultsEditQuery[0][0]: (resultsEditQuery[0][1],
                                           resultsEditQuery[0][2],
                                           resultsEditQuery[0][3]
                                           )}
    tablePrint(resultDict)
    print("Description: \n" + resultsEditQuery[0][4])
    pass
...
#          )
    ...
    while not menuSelection:
        ...
        if menuSelection == "q":
            ...
            print("\nSQL Results: ", checkForItem(conn, itemName))
